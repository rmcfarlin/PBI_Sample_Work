...
| **Baseball API** | [baseball_api](./reports/baseball_api/) | live [MLB Stats API](https://statsapi.mlb.com/api/v1/) | A refreshable star-schema model built directly on a live REST API via Power Query (`Web.Contents` → JSON), with a parameterized season. 4 pages — standings, dynamic hitting & pitching leaderboards (rank by any stat), and a player explorer — on the Ballpark design system. [Read the writeup →](./reports/baseball_api/README.md) |
